...
def prepare_lawyer_summary(availability_data, bios_data):
    ...
    available_lawyers = availability_data[
        availability_data['Do you have capacity to take on new work?'].str.lower().str.contains('yes|maybe', na=False)
    ]
    ...
    for _, row in available_lawyers.iterrows():
        ...
        if not bio_row.empty:
            ...
            summary = {
                'name': name,
                'availability_status': clean_text_field(row['Do you have capacity to take on new work?']),
...
def main():
    ...
    try:
        ...
        if st.checkbox("Show Debug Info"):
            ...
            st.write("Availability Data Sample:")
        ...
        if not lawyers_summary:
            st.error("No available lawyers found in the system. Please check the data files.")
            return
